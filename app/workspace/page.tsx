...
export default function WorkspacePage() {
  ...
  useEffect(() => {
    async function fetchFiles() {
      try {
        const res = await fetch(`/api/workspace/${selectedRepo}/files`)
        if (res.ok) {
          const data = await res.json()
          setFiles(data.files || [])
      ...
      <div className="p-8 space-y-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold text-white">
                {stats?.lastScan ? `${Math.floor((Date.now() - stats.lastScan) / 60000)}m` : '-'}
              </div>
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold text-white">
                {selectedRepo === 'self' ? 'Self' : repositories.find(r => r.id === selectedRepo)?.name || '-'}
              </div>
              <p className="text-xs text-zinc-500 mt-1">
                {selectedRepo === 'self' ? 'Prometheus' : 'External repo'}
              </p>
            </CardContent>
          </Card>
        </div>
